...
module.exports = class _Team {
    ...
    setName(newName){
        ...
        let val = this.val;
        val.name = newName;
        teams = teams[this.league].filter(it => it.name != this.val.name)
    ...
    delete(){
        teams = teams[this.league].filter(v => v.name != this.val.name);
        fs.writeFile('./storage/teams.json', JSON.stringify(teams), (err) => {
            if(err) console.log(err);
        })
    ...
    static createTeam(team, serverId){
        let league = _League.getLeague(serverId);
        let val = this.exists(team, league);
        if(val != null) return null;
        let json = {
