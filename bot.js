...
const { connectMongo } = require("./util/utils.js");
...
//call leagues from mongo
let leagues = connectMongo(async (db) => {
  //add collection to object
  const leagues = db.collection("leagues");
  //searches in the db the leagues.
  const cursorLeagues = await leagues.find({});
          ...
          if (err) console.log(err);
            ...
            data.forEach((val) => {
              if (val.contains(" was painted by ")) {
                ...
                else if (player1.team != obj.team1 && player1.team != obj.team2)
                  return new _NoticeEmbed(
                    Colors.ERROR,
                    painted + " is not on any team"
                  );
                ...
                else if (player2.team != obj.team1 && player2.team != obj.team2)
                  return new _NoticeEmbed(
                    Colors.ERROR,
                    painter + " is not on any team"
                  );
